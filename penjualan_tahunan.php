...
<body id="page-top">
        ...
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item active">
                <a class="nav-link" href="index.php">
                    ...
                    <span>Chart Penjualan Tahunan</span></a>
            </li>
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <?php
                                        ...
                                        $tot_penjualan = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',w.Tahun) as tahun, CONCAT('y:', SUM(fs.OrderQty)*100/" . $tot_penjualan[0] .") as y, CONCAT('drilldown:', w.Tahun) as drilldown 
                                        FROM dimensi_waktu w
                                        JOIN fakta_sales fs ON (w.DateID = fs.DateID) 
                                        GROUP BY tahun";
                                        $all_jual = mysqli_query($mysqli,$sql);
                                        while($row = mysqli_fetch_all($all_jual)) {
                                            $data[] = $row;
                                        }
                                        $json_all_jual = json_encode($data);

                                        // Chart Kedua

                                        $sql = "SELECT w.Tahun tahun, w.Bulan bulan, SUM(fs.OrderQty) as qty
                                        FROM dimensi_waktu w
                                        JOIN fakta_sales fs ON (w.DateID = fs.DateID)
                                        GROUP BY tahun, bulan";
                                        $det_jual = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_jual)) {
                                            $data_det[] = $row;
                                        }
    ...
    <script type="text/javascript">
            ...
            title: {
                text: 'Presentase Jumlah Penjualan Berdasarkan Tahun'},
                subtitle: {
                    text:'Klik untuk melihat jumah detail dari setiap bulan'
                },
                accessibility: {
                    announceNewData: {
                        enabled: true
