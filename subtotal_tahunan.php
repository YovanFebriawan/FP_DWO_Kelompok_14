...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Sub Total</h1>
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <?php
                                        ...
                                        $sql = "SELECT SUM(SubTotal) AS qty FROM fakta_pembelian";
                                        $tot = mysqli_query($mysqli, $sql);
                                        $tot_SubTotal = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',w.Tahun) as tahun, CONCAT('y:', SUM(fp.SubTotal)*100/" . $tot_SubTotal[0] .") as y, CONCAT('drilldown:', w.Tahun) as drilldown 
                                        FROM dimensi_waktu w
                                        JOIN fakta_pembelian fp ON (w.DateID = fp.DateID) 
                                        GROUP BY tahun";
                                        $all_sub = mysqli_query($mysqli,$sql);
                                        while($row = mysqli_fetch_all($all_sub)) {
                                            $data[] = $row;
                                        }
                                        $json_all_sub = json_encode($data);

                                        // Chart Kedua

                                        $sql = "SELECT w.Tahun tahun, w.Bulan bulan, SUM(fp.SubTotal) as qty
                                        FROM dimensi_waktu w
                                        JOIN fakta_pembelian fp ON (w.DateID = fp.DateID)
                                        GROUP BY tahun, bulan";
                                        $det_sub = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_sub)) {
                                            $data_det[] = $row;
                                        }
    ...
    <script type="text/javascript">
            ...
            title: {
                text: 'Presentase Sub Total Pengeluaran Berdasarkan Tahun'},
                subtitle: {
