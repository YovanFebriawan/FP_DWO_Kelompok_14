...
<body id="page-top">
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <?php
                                        ...
                                        $sql = "SELECT SUM(OrderQty) AS qty FROM fakta_sales";
                                        $tot = mysqli_query($mysqli, $sql);
                                        $tot_inventory = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',p.LocationID) as lokasi_id, CONCAT('y:', SUM(fs.OrderQty)*100/" . $tot_inventory[0] .") as y, CONCAT('drilldown:', p.LocationID) as drilldown 
                                        FROM dimensi_produk p
                                        JOIN fakta_sales fs ON (p.ProductID = fs.ProductID) 
                                        GROUP BY lokasi_id";
                                        $all_loc = mysqli_query($mysqli,$sql);
                                        while($row = mysqli_fetch_all($all_loc)) {
                                            $data[] = $row;
                                        }
                                        $json_all_loc = json_encode($data);

                                        // Chart Kedua

                                        $sql = "SELECT p.LocationID lokasi_id, p.Name name, SUM(fs.OrderQty) as qty
                                        FROM dimensi_produk p
                                        JOIN fakta_sales fs ON (p.ProductID = fs.ProductID)
                                        GROUP BY lokasi_id, name";
                                        $det_nama = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_nama)) {
                                            $data_det[] = $row;
                                        }
    ...
    <script type="text/javascript">
            ...
            title: {
                text: 'Presentase Jumlah Produk Berdasarkan ID Lokasi'},
                subtitle: {
                    text:'Klik untuk melihat detail nama dari setiap produk tersedia'
                },
                accessibility: {
                    announceNewData: {
                        enabled: true
                ...
                series: [
                    {
                        name: "Detail Nama Produk",
                        colorByPoint: true,
                        data:
                            <?php
                            $datanya = $json_all_loc;
