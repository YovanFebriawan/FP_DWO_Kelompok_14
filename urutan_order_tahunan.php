...
<body id="page-top">
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <?php
                                        ...
                                        $sql = "SELECT COUNT(PurchaseOrderID) AS poi FROM fakta_pembelian";
                                        $tot = mysqli_query($mysqli, $sql);
                                        $tot_transaksi = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',w.Tahun) as tahun, CONCAT('y:', COUNT(fp.PurchaseOrderID)*100/" . $tot_transaksi[0] .") as y, CONCAT('drilldown:', w.Tahun) as drilldown 
                                        FROM dimensi_waktu w
                                        JOIN fakta_pembelian fp ON (w.DateID = fp.DateID) 
                                        GROUP BY tahun";
                                        $all_tran = mysqli_query($mysqli,$sql);
                                        while($row = mysqli_fetch_all($all_tran)) {
                                            $data[] = $row;
                                        }
                                        $json_all_tran = json_encode($data);

                                        // Chart Kedua

                                        $sql = "SELECT w.Tahun tahun, w.Bulan bulan, COUNT(fp.PurchaseOrderID) as qty
                                        FROM dimensi_waktu w
                                        JOIN fakta_pembelian fp ON (w.DateID = w.DateID)
                                        GROUP BY tahun, bulan";
                                        $det_tran = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_tran)) {
                                            $data_det[] = $row;
                                        }
    ...
    <script type="text/javascript">
            ...
            title: {
                text: 'Presentase Jumlah Transaksi Berdasarkan Tahun'},
                subtitle: {
