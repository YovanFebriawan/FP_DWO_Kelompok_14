...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Line Total</h1>
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <?php
                                        ...
                                        $sql = "SELECT SUM(LineTotal) AS lt FROM fakta_sales";
                                        $tot = mysqli_query($mysqli, $sql);
                                        $tot_line = mysqli_fetch_row($tot);

                                        $sql = "SELECT CONCAT('name:',w.Tahun) as tahun, CONCAT('y:', SUM(fs.LineTotal)*100/" . $tot_line[0] .") as y, CONCAT('drilldown:', w.Tahun) as drilldown 
                                        FROM dimensi_waktu w
                                        JOIN fakta_sales fs ON (w.DateID = fs.DateID) 
                                        GROUP BY tahun";
                                        $all_line = mysqli_query($mysqli,$sql);
                                        while($row = mysqli_fetch_all($all_line)) {
                                            $data[] = $row;
                                        }
                                        $json_all_line = json_encode($data);

                                        // Chart Kedua

                                        $sql = "SELECT w.Tahun tahun, w.Bulan bulan, SUM(fs.LineTotal) as lt
                                        FROM dimensi_waktu w
                                        JOIN fakta_sales fs ON (w.DateID = fs.DateID)
                                        GROUP BY tahun, bulan";
                                        $det_line = mysqli_query($mysqli,$sql);

                                        while ($row = mysqli_fetch_all($det_line)) {
                                            $data_det[] = $row;
                                        }
    ...
    <script type="text/javascript">
            ...
            title: {
                text: 'Presentase Jumlah Line Total Berdasarkan Tahun'},
                subtitle: {
                    text:'Klik untuk melihat line total dari setiap bulan'
                },
                accessibility: {
                    announceNewData: {
                        enabled: true
